...
Usage:
    python -m evaluation.run_evals [--tickets N] [--output results.json]
...
def run_evaluation(gold_data: list, verbose: bool = False) -> list:
    ...
    for i, item in enumerate(gold_data):
        ...
        try:
            # Step 1: Ensure KB is ingested
            if i == 0:
                ingest_resp = requests.post(f"{API_BASE}/knowledge-base/ingest")
                if verbose:
                    print(f"  KB ingest: {ingest_resp.json()}")
            
            # Step 2: Create ticket
            create_resp = requests.post(f"{API_BASE}/tickets", json={"description": description})
            ...
            search_resp = requests.get(f"{API_BASE}/documents/search", params={"query": description, "limit": 3})
...
def main():
    ...
    if summary.get("avg_keyword_coverage", 0) < 0.5:
